...
def getSegsWithLabelsShape(tupl):
    labPath = tupl[1]
    imageLab = sitk.ReadImage(join( labelsPath,labPath))

    segPath = tupl[0]
    imageSeg = sitk.ReadImage(join( segmDir,segPath))
    sizeA = imageLab.GetSize()
    sizeB = imageSeg.GetSize()

    return (sizeA[0]==sizeB[0] and sizeA[1]==sizeB[1] and sizeA[2]==sizeB[2] )
...
zipped= list(filter(getSegsWithLabelsShape ,zipped))
# zipped= zipped[0:90]#list(filter(getSegsWithLabelsShape ,zipped))
...
def getValidScore(segmToEvall,labelsToEval):
    # zipped=list(zip(segmToEvall,labelsToEval ))
    # zipped= list(filter(getSegsWithLabelsShape ,zipped))
    # segmToEvall= list(map(lambda el: el[0],zipped))
    # labelsToEval= list(map(lambda el: el[1],zipped))
...
def standardizeFile(maskFilepath):
    if(exists(maskFilepath)):
        ...
        origArrOrig = origArr.copy()
        origArr[np.logical_not(imageSegArr)]=0
        if(np.sum(origArrOrig)> np.sum(origArr)):
            print("chaaanged in  standardizeFile   ")
        ...
        sizeA = imageSeg.GetSize()
        sizeB = lbl_new.GetSize() 
        if(not (sizeA[0]==sizeB[0] and sizeA[1]==sizeB[1] and sizeA[2]==sizeB[2] )):  
            print("  wrong sizes in standardizeFile ")        
    ...
    if(exists(maskFilepath)):
        ...
        if(row['isError']):
            ...
            if(currValue<min or currValue>max):
                ...
                imageSegArrPrim= imageSegArr.copy()
                imageSegArr = np.where( imageSegArr!=labNum, imageSegArr,0 )#(imageSegArr==labNum)
                print(f" modyfing arr prim {np.sum(imageSegArrPrim)} after {np.sum(imageSegArr)} ")
        ...
        sizeB = lbl_new.GetSize() 
        if(not (sizeA[0]==sizeB[0] and sizeA[1]==sizeB[1] and sizeA[2]==sizeB[2] )):  
            print("  wrong sizes in modifyFile ")
...
for modalityPair in modalities:
    ...
    for currentCol in minMaxess['name'].to_numpy():
        ...
        with mp.Pool(processes = mp.cpu_count()) as pool:
            tempPaths=pool.map( partial(modifyFile,minMaxesDf=minMaxess,currentCol=currentCol),rowws)
        tempPaths=list(filter(lambda ell:ell!=' ',tempPaths))
